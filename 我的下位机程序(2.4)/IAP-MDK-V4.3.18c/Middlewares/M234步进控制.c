	...
	*/
		...
		if(pusRegHoldingBuf[2]==1)/*右复位*/
	{
		if(plc_motor_cur_status(g_plc_flow_test_motor_port.id) == 0){
			...
			/*蜂鸣器函数*/
	if(pusRegHoldingBuf[2]==2)//向左移动1mm
	{
	...
	{			if(plc_motor_cur_status(g_plc_flow_test_motor_port.id) == 0){
		...
		if(pusRegHoldingBuf[2]==5)//向左移动279mm
	{
